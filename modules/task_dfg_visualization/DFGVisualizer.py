...
from palettable.colorbrewer.sequential import Purples_5_r, Oranges_5
...
class DFGVisualizer:
    ...
    def __init__(self, graph, password, name_data_set, entity_labels, action_lifecycle_labels, analysis_directory,
                 clustering_instance, exclude_clusters=None):
        ...
        self.abbr_dict_lpm = data_set_dictionaries.abbr_dict_lpm[self.name_data_set]
        self.exclude_clusters = exclude_clusters

    def visualize_intra_task_DFG(self, task_clusters=""):
        if task_clusters == "":
            with self.driver.session() as session:
                task_clusters = session.read_transaction(query_task_cluster_list)
        for cluster in task_clusters:
            ...
            output_directory = os.path.join(self.analysis_directory, "intra_task_visualization", "DFG")
            os.makedirs(output_directory, exist_ok=True)
            dot.render(f'{output_directory}\\DFG_task_{cluster}', view=True)

    def visualize_inter_task_DFG(self, entity_type, df_show_threshold, df_weight_threshold, start_end_date=None,
                                 resources=None, print_description=False):
        dot = Digraph(comment='Query Result')
        ...
        with self.driver.session() as session:
            ...
            session.read_transaction(self.get_DFG, dot, dfg_id, task_instance_ids, entity_type, df_show_threshold,
                                     df_weight_threshold, print_description=print_description)
    ...
    def visualize_inter_task_DFG_concept_drift_comparison(self, entity_type, df_show_threshold, start_end_dates):
        dot = Digraph(comment='Query Result')
    ...
    def visualize_inter_task_DFG_resources(self, df_show_threshold_case, df_show_threshold_resource,
                                           resources_lists_over, start_end_date=None, resources=None):
        dot = Digraph(comment='Query Result')
    ...
    def get_DFG(self, tx, dot, dfg_id, task_instance_ids, entity_type, edge_show_threshold, edge_weight_threshold,
        ...
        for record in edge_results:
            if record['abs_freq'] > edge_show_threshold:
                ...
                edge_color, edge_font_color = get_medium_and_dark_color(True, 'case', record['abs_freq'],
                                                                        ...
                                                                        dfg_id=0)
                if record['abs_freq'] > edge_weight_threshold:
                    edge_label = str(record['abs_freq'])
                else:
                    edge_label = ""
                    edge_color = 'lightgrey'
                dot.edge(n1_id, n2_id, xlabel=edge_label, penwidth=f"{edge_weight}", color=edge_color,
        ...
        for record in node_results:
            if f"{record['cluster']}" in node_ids:
                ...
                if print_description:
                    node_description = f"{self.cluster_descriptions[record['cluster']]}<br/>{record['abs_freq']}>"
                else:
                    node_description = f"<<b>{record['cluster']}</b><br/><FONT POINT-SIZE=\"{str(font_size_small)}\">{record['abs_freq']}</FONT>>"
        ...
        dot.node(f"start_{dfg_id}",
                 f"<<b>start</b><br/><FONT POINT-SIZE=\"{str(font_size_small)}\">{start_count}</FONT>>", node='source')
        dot.node(f"end_{dfg_id}", f"<<b>end</b><br/><FONT POINT-SIZE=\"{str(font_size_small)}\">{end_count}</FONT>>",
                 node='sink')
        ...
        for index, row in df_node_results_joint.iterrows():
            ...
            if node_id in node_ids:
                ...
                if node_id in [f'start_{dfg_id}', f'end_{dfg_id}']:
                    ...
                    node_description = f"<<b>{row['cluster']}</b><br/><FONT COLOR=\"{freq_font_color}\" POINT-SIZE=\"{str(font_size_small)}\">{int(row['before_freq'])}<br/>{count_difference_string}</FONT>>"
                dot.node(node_id, node_description, fillcolor=node_fill_color, fontcolor=node_font_color)
        pr.record_performance('draw_nodes')
    ...
    def get_DFG_resources_overlaid(self, tx, dot, dfg_ids, list_task_instance_ids, df_show_threshold_case,
        ...
        for index, row in df_node_results_over.iterrows():
            node_id = str(row["n"])
            if node_id in node_ids:
                _, node_fill_color = get_medium_and_dark_color(False, 'resource', row['abs_freq'], df_show_threshold,
                                                               dfg_id=int(row['dfg_id']))
                _, node_font_color = get_medium_and_dark_color(True, 'resource', row['abs_freq'], 0,
                                                               dfg_id=int(row['dfg_id']))
                ...
                if node_id[:node_id.index('_')] == 'start':
                    ...
                    node_description = f"<<b>{row['n'][:row['n'].index('_')]}</b><br/><FONT COLOR=\"{node_font_color}\" POINT-SIZE=\"{str(font_size_small)}\">{int(row['abs_freq'])}</FONT>>"
                dot.node(node_id, node_description, fillcolor=node_fill_color, fontcolor=node_font_color)
        pr.record_performance('draw_nodes')
        return dot
    ...
    def get_intra_cluster_DFG(self, tx, dot, cluster):
        ...
        dot.attr("node", shape="rectangle", fixedsize="false", fontname="Helvetica",
                 fontsize=str(font_size_large), margin="0.2", color=black, style="rounded,filled", fillcolor=white,
                 fontcolor=black, penwidth="3")
        dot.attr("edge", color=color_dict['grey']['medium'], penwidth="2", fontname="Helvetica",
                 fontsize=str(font_size_small),
...
def query_inter_cluster_DFG_edges_min_max(tx, cluster, grey_threshold):
    ...
    if max_edge_freq < grey_threshold:
        max_edge_freq = grey_threshold + 2
...
def query_task_cluster_list(tx):
    q = f'''
        MATCH (tc:TaskCluster)
        WITH DISTINCT tc.Name AS task_cluster
        RETURN task_cluster
        '''
    result = tx.run(q)
    task_cluster_list = []
    for record in result:
        task_cluster_list.append(record['task_cluster'])
    return task_cluster_list
...
def get_file_name_resources(resources):
    ...
    for resource in resources:
        ...
        file_name_resources += f"_{resource}"
