...
class EventGraphConstructor:
    ...
    def construct(self, use_apoc=True):
        ...
        if use_apoc:
            ...
            construct(self.driver, self.data_entities, pr)

        pr.record_total_performance()
        pr.save_to_file()


def construct_apoc(driver, data_entities, performance_recorder):
...
def construct(driver, data_entities, performance_recorder):
    for entity in data_entities:
        query_create_entity_nodes = f'''
            MATCH (e:Event)
